...
func GetGithubRelease(url string) (*GithubRelease, error) {
	...
	if res.StatusCode >= 300 {
		...
		if isRateLimitedOrBlocked {
			...
			return &GithubRelease{}, errors.New("Rate limited or blocked")
		}
...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("CUMCORD_DEV_INSTALL") == "1"
	...
	Log.Debug("Found existing CumCord Install. Checking for hash...")
	scanner := bufio.NewScanner(f)
	if scanner.Scan() {
		line := scanner.Text()
		if strings.HasPrefix(line, "// CumCord ") {
